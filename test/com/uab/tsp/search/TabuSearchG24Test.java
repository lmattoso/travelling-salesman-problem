...
public class TabuSearchG24Test {
    ...
    public void variableRandomSeed() throws InterruptedException {
        ...
        List<City> cities = citiesReader.getCitiesFromFile("cities/gr24.xml");
        randomSeed = 1L;
        isFrequencyBasedMemory = false;
        stochasticSample = true;
        neighbourPerc = 0.95;
        maxTriesMove = 100;
        final int minCost = 1272;
        ...
        assertEquals(minCost, results.getSolution().cost().doubleValue(), 0);
    }
    ...
    public void simulation() throws InterruptedException {
        ...
        for(double d = 0.2 ; d <= 0.99; d += 0.01) {
            ...
            for(int m = 10; m < 100; m+=5) {
                ...
                if( minCost  >= results.getSolution().cost().intValue() * 0.85 && results.getSolution().costLessThan(best) ) {
